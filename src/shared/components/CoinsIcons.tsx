...
export const CoinsIcons: React.FC<Props> = ({ coins, maxShow, size }) => {
  const [_coins, isMaxShowEnable] = useMemo(() => {
    const coinsArray = Array.isArray(coins) ? coins : coins.split("#").map((c) => c.split("_")[1]?.toUpperCase());
  ...
  return (
    <Avatar.Group>
      ...
      {isMaxShowEnable && (
        <p className="ml-2 ms-2 flex items-center text-white" style={{ marginInlineStart: "0.5rem" }}>
          . . .
        </p>
      )}
    </Avatar.Group>
