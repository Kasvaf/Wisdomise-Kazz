...
export function toMoneyBigInt(src: bigint | number | string, pow10: number) {
  const ten10 = 10n ** BigInt(pow10);
  if (typeof src === 'bigint') {
    ...
    if (frac.length > pow10) {
      throw new Error('Invalid number');
    }
    while (frac.length < pow10) {
      frac += '0';
    }

    // Convert
    const r = BigInt(whole) * ten10 + BigInt(frac);
    return neg ? -r : r;
  }
}
