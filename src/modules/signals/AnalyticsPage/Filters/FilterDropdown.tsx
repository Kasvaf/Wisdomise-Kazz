...
const filterToSelectionRange = (
  filter: Record<string, Date | boolean | string | Dayjs>,
) => ({
...
function FilterDropdown({
  ...
  const handleDateSelect = (ranges: Record<string, Range>) => {
    const newFilter = cloneDeep(filter);
    if (ranges.range1.startDate == null) return;
    newFilter[name] = {
      start: ranges.range1.startDate,
      end: ranges.range1.endDate != null || ranges.range1.startDate,
    };
    updateFilter(newFilter);
  };
  ...
  const activeFilterCount = useMemo(
    () =>
      Object.values(filter[name]).filter(val =>
        isArray(val) ? val.length : val,
      ).length,
    [filter, name],
  );
      ...
      overlay={
        ...
        >
          {(type === 'singleselect' || type === 'multiselect') &&
            ...
            Object.keys(options)
              .sort((a, b) =>
                disabledItems?.includes(b as DisableItemType) ? -1 : 1,
              )
              .map(o => (
                <button
