    ...
    >
      ...
      {socials?.map(social => (
        ...
        >
          <span
            className={clsx(
              'shrink-0 rounded-full bg-white/10 text-white/60 text-xxs transition-all hover:text-white/60 active:brightness-90',
              size === 'xs' &&
              ...
              size === 'md' &&
                '[&_svg]:!size-[12px] flex h-6 items-center justify-center gap-1 px-3 [&_img]:size-[12px]',
            )}
            onClick={e => {
              e.preventDefault();
              e.stopPropagation();
