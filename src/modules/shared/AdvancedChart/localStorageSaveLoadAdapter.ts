...
export class LocalStorageSaveLoadAdapter implements IExternalSaveLoadAdapter {
  private readonly _charts: SavedChartData[] = [];
  private readonly _studyTemplates: StudyTemplateData[] = [];
  private readonly _drawingTemplates: DrawingTemplate[] = [];
  ...
  public getStudyTemplateContent(
    ...
    for (const template of this._studyTemplates) {
  ...
  public loadDrawingTemplate(
    ...
    for (const template of this._drawingTemplates) {
      if (template.name === templateName && template.toolName === toolName) {
        return Promise.resolve(template.content);
      }
    }
    return Promise.reject(new Error('The drawing template does not exist'));
  ...
  public getDrawingTemplates(): Promise<string[]> {
    return Promise.resolve(
      this._drawingTemplates.map((template: DrawingTemplate) => template.name),
    );
  }
