    ...
    <>
      <div className="flex justify-around gap-2 rounded-lg bg-white/5 p-4 mobile:bg-black/5">
        <div className="flex flex-col items-center">
          <p className="text-xs text-white/80 mobile:text-black/80">Total Balance</p>
          <p className="text-white mobile:text-black">
            {roundDown(totalBalance)} <span className="text-white/80 mobile:text-black/80">BUSD</span>
          </p>
        </div>
        <div className="border-l border-white/10" />
        <div className="flex flex-col items-center ">
          <p className="text-xs text-white/80 mobile:text-black/80">Withdrawable</p>
          <p className="text-white mobile:text-black">
            {roundDown(withdrawable)} <span className="text-white/80 mobile:text-black/80">BUSD</span>
          </p>
        </div>
      </div>
      {(ias.data?.[0]?.financial_product_instances.length || 0) > 0 &&
        ias.data?.[0]?.financial_product_instances[0].status !== "DRAFT" && (
          <p className="mt-2 px-2 text-center text-xs  text-white/80 mobile:text-black/80">
            You Have <span className="text-white mobile:text-black">1</span> Running Product <br /> That Block{" "}
            <span className="text-white mobile:text-black">{roundDown(totalBalance - withdrawable)}</span> BUSD Of Your
            Equity
          </p>
        )}
